...
oauth2_schema_customer = OAuth2PasswordBearer(tokenUrl='customer_authentification/login')
...
def get_current_user(token: str = Depends(oauth2_schema_user), db: Session = Depends(get_db)):
...
def get_current_customer(token: str = Depends(oauth2_schema_customer), db: Session = Depends(get_db)):
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Could not validate credentials",
        headers={"www-Authenticate": "Bearer"}
